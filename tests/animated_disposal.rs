...
#[test]
#[ignore = "image-webp 0.2.4 does not correctly clear disposed frame rectangles"]
fn webp_disposal() {
    check_fixture("disposal.webp");
}

#[test]
#[ignore = "image-webp 0.2.4 does not correctly clear disposed frame rectangles"]
fn webp_alpha() {
    check_fixture("alpha.webp");
}

#[test]
#[ignore = "image-webp 0.2.4 does not correctly clear disposed frame rectangles"]
fn webp_disposal_edges() {
    check_fixture("disposal-edges.webp");
}
...
fn check_fixture(name: &str) {
    ...
    // Cached output must preserve compositing, alpha flattening and frame delays.
    ...
    for ((rgb, delay), expected) in cold.frames.iter().zip(&golden.frames) {
        ...
        assert_eq!(rgb.len(), (golden.width * golden.height * 3) as usize);
